...
import type { MarketCode } from "@/lib/commerce/markets-config"
...
const SHIPPING_LABEL_KEYS: Partial<Record<string, TranslationKey>> = {
  omniva: "omnivaParcel",
  dpd: "dpdPickup",
  venipak: "venipakParcel",
  smartpost: "smartpostItella",
  courier: "courierDelivery",
}

export interface SerializedCheckoutShippingOption {
  id: string
  name: string
  priceCents: number
  days: string
...
export interface CheckoutContentProps {
  shippingOptions: SerializedCheckoutShippingOption[]
  vatRateBps: number
...
function shippingLabel(
  option: SerializedCheckoutShippingOption,
  t: (key: TranslationKey) => string
): string {
  if (option.id === "courier") {
    return "Kurjera piegāde"
  }
  const key = SHIPPING_LABEL_KEYS[option.id]
...
export function CheckoutContent({
  ...
  const shippingCost = shippingOption ? eur(shippingOption.priceCents) : eur(shippingOptions[0]?.priceCents ?? 350)
  ...
  const vatAmount = eur(extractInclusiveVatCents(finalTotal.amount, vatRateBps))
            ...
            <div className="mb-6 rounded-lg bg-muted p-4 text-left">
              ...
              <p className="font-medium text-card-foreground">
                {isCourier
                  ...
                  : `${shippingOption ? shippingLabel(shippingOption, t) : ""}: ${LATVIAN_STATIONS.find(s => s.id.toString() === selectedStation)?.name}`
                }
              </p>
                    ...
                    {shippingOptions.map((option) => (
                      ...
                      >
                        ...
                        <div className="flex-1">
                          <p className="font-medium text-card-foreground">
                            {shippingLabel(option, t)}
                          </p>
                          <p className="text-sm text-muted-foreground">Piegāde {option.days} darba dienas</p>
                        </div>
                        <span className="font-semibold text-primary">{formatMoney(eur(option.priceCents))}</span>
                      </label>
                    ))}
                  </div>
                </section>
