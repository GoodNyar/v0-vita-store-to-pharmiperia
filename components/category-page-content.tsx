...
export function CategoryPageContent({
  ...
  const catalogData = useMemo(() => {
    ...
    if (!cat) {
      return { category: null as Category | null, products: [] as Product[], brands: [] as Brand[] }
    }

    const catName = t(cat.id as TranslationKey)
    ...
    const products: Product[] = catalogProducts.map((p) => ({
        id: String(p.id),
        sku: p.sku ?? "",
        name: p.name,
    ...
    const brands: Brand[] = Array.from(
      new Map(products.map((p) => [p.brand.slug, p.brand])).values()
    ).map((b) => ({ id: b.slug, name: b.name, slug: b.slug }))

    return { category, products, brands }
  }, [slug, t, catalogProducts])
